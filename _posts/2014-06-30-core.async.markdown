...
Je conseille de travailler avec des _channels_ non bufferisés dans un premier temps. Cela rend le raisonnement et la compréhension du code plus aisés.
...
A ce stade, `player1` contient le vecteur `[5 8]`. Par contre l'exécution du programme est bloquée jusqu'à ce que la valeur du _channel_ soit lue. Il existe bien une opération de lecture `<!!`, mais puisqu'on est bloqué, on ne pourra jamais l'exécuter. Comment faire ?
...
Notons que la valeur valeur actuelle du score est complètement *encapsulée*. Personne d'autre ne peut connaitre le score.
...
Je pense qu'on peut appeler chaque thread un acteur. Au pire on peut dire que ce sont des objets, dans le sens où ils encapsulent un état et communique par message. 
...
Cela me fait penser au modèle de [Vert.x](http://vertx.io).
...
Comme nous l'avons vu, nous n'avons pas affaire à la lourdeur d'un framework qui vient avec une façon de faire, mais une librairie qui permet de faire du *sur mesure*. 
